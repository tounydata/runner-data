    ...
    >
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
        ...
        {activity.total_elevation_gain > 0 && (
          <Stat value={`+${String(Math.round(activity.total_elevation_gain))}`} label="D+" />
        )}
      </div>

      {activity.average_heartrate && (
        <div style={{ fontFamily: 'var(--mono)', fontSize: '.6rem', color: 'var(--red)' }}>
          ♥ {String(Math.round(activity.average_heartrate))} bpm moy
        </div>
      )}
      ...
      <button
        onClick={() => {
          void navigate(`/analysis?activityId=${String(activity.id)}`)
        }}
        style={{
          width: '100%',
          marginTop: 4,
