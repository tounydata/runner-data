...
export function DashboardPage() {
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '60vh', gap: '1.5rem', textAlign: 'center', padding: '2rem' }}>
        ...
        <button
          ...
          style={{
            ...
            fontSize: '.88rem', cursor: 'pointer', maxWidth: 300, width: '100%',
          }}
        >
          Connecter Strava →
        </button>
  ...
  const daysToNext = nextRace ? Math.ceil((new Date(nextRace.date).getTime() - now.getTime()) / 86400000) : null
    ...
    <div>
      ...
      <div
        style={{
          ...
          background: 'var(--border)', borderRadius: 'var(--r)',
          overflow: 'hidden', marginBottom: '1.25rem',
        }}
      >
        {[
          { value: metresToKm(kmMonth), label: 'km / mois' },
          { value: thisMonth.length.toString(), label: 'sorties' },
          { value: `+${Math.round(elevMonth)}`, label: 'D+ mois' },
          { value: runs.length.toString(), label: 'total runs' },
          { value: nextRace ? `J-${daysToNext ?? 0}` : '—', label: 'prochain' },
        ].map((cell) => (
          <div key={cell.label} style={{ background: 'var(--bg2)', padding: '12px 16px', textAlign: 'center' }}>
            <div style={{ fontFamily: 'var(--display)', fontSize: '1.4rem', letterSpacing: '.03em', lineHeight: 1, color: 'var(--cyan)' }}>
              {cell.value}
            </div>
            <div style={{ fontFamily: 'var(--mono)', fontSize: '.54rem', color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '.08em', marginTop: 3 }}>
              {cell.label}
            </div>
          </div>
        ))}
      ...
      {nextRace && (
        <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 'var(--r)', padding: '1.25rem', marginBottom: '1.25rem' }}>
          <div style={{ fontFamily: 'var(--mono)', fontSize: '.56rem', color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '.1em', marginBottom: '.5rem' }}>
            Prochaine course
          </div>
          <div style={{ fontFamily: 'var(--display)', fontSize: '4rem', lineHeight: 1, color: 'var(--cyan)' }}>
            {daysToNext}
          </div>
      ...
      <div style={{ fontFamily: 'var(--mono)', fontSize: '.56rem', color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '.1em', marginBottom: '1rem' }}>
        Dernières activités
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(265px, 1fr))', gap: 10 }}>
        {runs.slice(0, 9).map((a) => (
          <div key={a.id} style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 9, padding: '12px 14px' }}>
            ...
            <div style={{ fontFamily: 'var(--mono)', fontSize: '.7rem', color: 'var(--text2)' }}>
              {metresToKm(a.distance)} km · +{Math.round(a.total_elevation_gain)} m
            </div>
          </div>
        ))}
      </div>
